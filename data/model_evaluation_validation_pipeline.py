...
def genInferenceData():
    ...
    GenInferenceData Algorithm:
        ...
        * The annotations file has, for each image_id, the associated tile number and chip co-ordinates encoded into the image name.
        ...
        * Once this hashMap is prepared, we are ready to iterate on a tile by tile basis:
            ...
            * Essentially: We have a mapping between the tile and image_ids for all chips in the tile, for all tiles.
            * For each chip:
    ...
    for i,tile in enumerate(tileMap.keys()):
        ...
        if(len(tileMap[tile])!=expectedChips):
            ...
            infTile = np.zeros((rows*chipShape[0],cols*chipShape[1]),dtype=np.uint8)
